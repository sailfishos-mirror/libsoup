...
typedef struct {
	char *filename;
	sqlite3 *db;
	guint db_default_page_size;
	guint db_default_max_page_count;
	guint max_size;
...
static gboolean db_set_max_size (SoupCookieJarDB *jar, guint max_size, GError **error);
...
read_default_max_page_count_callback (void *data, int argc, char **argv, char **colname)
{
	...
	priv->db_default_max_page_count = strtoul (argv[0], NULL, 10);
	return 0;
}
...
read_default_page_size_callback (void *data, int argc, char **argv, char **colname)
{
	...
	priv->db_default_page_size = strtoul (argv[0], NULL, 10);
	return 0;
}
 ...
 **/
gboolean
soup_cookie_jar_db_set_max_size (SoupCookieJarDB *jar, guint max_size, GError **error)
{
...
static gboolean
db_set_max_size (SoupCookieJarDB *jar, guint max_size, GError **error)
{
	SoupCookieJarDBPrivate *priv =
	...
	guint max_page_count = (max_size != 0) ? max_size / priv->db_default_page_size : priv->db_default_max_page_count;
